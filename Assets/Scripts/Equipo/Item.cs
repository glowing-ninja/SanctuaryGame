...
public class Item {
	...
	public static Item ItemGenerator(int type, int subType, int level, int chest) {
		Item i = null;
		int mod = 0;
		if (chest == 2) {
			mod = 10;
		...
		if (type == 0) {		// Si es un arma calculamos el subtip
			subType = UnityEngine.Random.Range(0,5);
			switch(subType) {
